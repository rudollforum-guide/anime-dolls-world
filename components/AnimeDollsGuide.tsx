...
const materials = [
  ...
  ["S-TPE / STPE", "Фирменные обозначения, встречающиеся у Irokebijin и Croissant Doll. Это не единый отраслевой стандарт."],
...
const technologies = [
  ...
  { name: "Elsa Babe", features: ["Anime Electronic Eyes", "AI Voice Function", "Articulated Fingers", "Ultra-Soft Body", "Gel Butt"] },
...
export function AnimeDollsGuide() {
  const webPageJsonLd = {
    ...
    description: "Обзор аниме-кукол: основные бренды, материалы, головы, глаза, каркасы и особенности кастомизации.",
    url: `${siteConfig.url}/anime-dolls/`,
    isPartOf: { "@type": "WebSite", name: siteConfig.name, url: siteConfig.url },
  };

  return <>
    <PageHero eyebrow="Аниме-направление" title="Аниме-куклы" description="Стилизованные модели, оригинальные персонажи и образы по мотивам игр, аниме и манги — от компактных фигур до полноразмерных кукол с расширенной кастомизацией." crumbs={[{ label: "Аниме-куклы" }]} />
    <article className="anime-guide">
      <section className="section"><div className="container anime-intro"><div><p className="eyebrow">Общее направление</p><h2>Что такое аниме-куклы</h2><p>Аниме-куклы отличаются не только крупными глазами и стилизованными чертами лица. Современные производители используют разные подходы к переносу двухмерного персонажа в физическую форму: ручной и decal-макияж, сменные головы и глаза, персонажные парики, специальные пропорции тела, готовые костюмы и индивидуальную кастомизацию.</p></div><div className="anime-intro-list"><h3>В каталогах встречаются</h3><ul>{["оригинальные персонажи производителей", "классические аниме-образы", "модели по мотивам игр, аниме и манги", "кастомные проекты по изображениям заказчика", "компактные коллекционные модели", "полноразмерные модели", "торсы и отдельные головы"].map(item=><li key={item}>{item}</li>)}</ul></div><aside className="anime-note">Наличие конкретного персонажа или образа может меняться. Права на персонажей и статус официальной лицензии необходимо проверять отдельно.</aside></div></section>
