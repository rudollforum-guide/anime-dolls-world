...
const comparisons = [
  ...
  { name: "Dolls Castle", points: ["fantasy / anthropomorphic направление", "пока ограниченный объём подтверждённых технических данных"] },
];
...
export function FurryDollsGuide() {
  const webPageJsonLd = {
    ...
    description: "Обзор фурри- и антропоморфных кукол: Moonvale Doll, Elsa Babe, Dolls Castle, конструкция, совместимость и выбор.",
    url: `${siteConfig.url}/furry-dolls/`,
    isPartOf: { "@type": "WebSite", name: siteConfig.name, url: siteConfig.url },
  };

  return <>
    <PageHero className="furry-dolls-page-hero" eyebrow="Фурри-направление" title="Фурри-куклы" description="Антропоморфные, звериные и фантазийные персонажи с нестандартными головами, ушами, хвостами и другими декоративными элементами." crumbs={[{ label: "Фурри-куклы" }]} />
    <article className="furry-guide">
      <section className="section"><div className="container furry-intro"><div><p className="eyebrow">Отдельная эстетика</p><h2>Что такое фурри-куклы</h2><p>Фурри-куклы — отдельное направление стилизованных моделей, в котором человеческие пропорции сочетаются с животными или фантазийными чертами.</p><aside>Furry не является единым техническим стандартом. У разных производителей под этим термином могут подразумеваться очень разные конструкции.</aside></div><div><h3>В дизайне могут использоваться</h3><ul>{visualElements.map(item=><li key={item}>{item}</li>)}</ul></div></div></section>
